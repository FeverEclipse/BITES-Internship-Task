...
class AnalogSpeedometer(Canvas):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.center_image = Image.open("assets/speedobg.png").resize([220,220])
        self.rotating_image = Image.open("assets/pointy.png").resize([220,220])
        ...
        self.angle_text = self.create_text(self.center_x, self.center_y, text="0", fill="white", font=("Segoe UI Bold", 24))

        '''self.rotate_thread = threading.Thread(target=self.rotate_loop, daemon=True)
        self.rotate_thread.start()'''

    def rotate_needle(self, angle):
        rotated_image = self.rotating_image.rotate(-1 * angle * 270/360 * 2 / 5 if angle != 0 else 0, expand=True)
        ...
        self.itemconfig(self.angle_text, text=f"{int(angle)}")
        self.update()
    ...
    '''def rotate_loop(self):
        ...
        for angle in range(50, 361):
            if angle < 90:
                ...
                self.itemconfig(self.angle_text, text=f"{int((360 - angle)*900/270)}")
            else:
